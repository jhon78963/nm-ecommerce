...
export async function GET() {
  try {
    ...
    return proxyAuthenticatedGateway(
      `/ecommerce/customer/cart?warehouse_id=${encodeURIComponent(warehouseId)}`,
      { method: "GET" },
    );
  } catch (error) {
...
export async function PUT(request: Request) {
  try {
    ...
    return proxyAuthenticatedGateway("/ecommerce/customer/cart", {
      method: "PUT",
      body: JSON.stringify({ ...body, warehouseId }),
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : "No se pudo guardar el carrito.";
    return NextResponse.json({ message }, { status: 500 });
  }
}

export async function DELETE() {
  return proxyAuthenticatedGateway("/ecommerce/customer/cart", { method: "DELETE" });
}
